...
**Updated:** 2026-07-19 (Session 4 — Phase 0 close: 0d built; 0c run live to the Tier-1 endpoint, blocked on credits only; 0a/0e CI-proven; F-025/F-026 ledgered) · prior: Session 3 same day (0e CVE gate wired) · 2026-07-18 (Session 2 — Phase 0 build, locks 0a/0b/0c) · **Maintainer:** the loop; refreshed at every checkpoint.
...
1. **Fund the API account to finish 0c** (D-05 residual, now billing-only): the key injection question is solved (`MY_ANTHROPIC_API_KEY` injected in-container Session 4; the reserved-name strip documented — never store under `ANTHROPIC_API_KEY` in claude.ai/code env settings) and the full guarded path is proven live — the endpoint authenticated the key and answered **400 "credit balance is too low"**. Add credits at the provider console, then re-run: `./target/release/holmes-smoke --goose /home/user/goose-src/target/release/goose --provider anthropic --model claude-sonnet-5 --credential-env MY_ANTHROPIC_API_KEY --transcript <path>` → expect exit 0, verdict ROUND-TRIP COMPLETE; paste the transcript beside the existing one and flip 0c to COMPLETE. (goose is container-ephemeral: rebuild via `scripts/build-goose.sh` in a fresh container, ~12 min.)
...
**Phase 0 close (Session 4):** 0a CI-proven · 0b green (49/49 workspace suite) · **0c all guard/protocol legs proven live to the Tier-1 endpoint — one human action (account credits) from COMPLETE** · **0d COMPLETE** (`holmes-core` embedding contract) · 0e CVE gate proven in CI (provenance/attestation still open). Remaining Phase 0 items, all human-gated or trivial: (a) fund the API account → re-run 0c → flip to COMPLETE; (b) mark `acdl-gate` + `supply-chain` required checks on `main`; (c) 0e provenance/attestation carries into RC prep. Next build phase after close: **Phase 1 — Analytical core** (loop §6). Every fresh session: `git fetch origin` first; goose is container-ephemeral (`scripts/build-goose.sh`).
