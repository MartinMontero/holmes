...
**Updated:** 2026-07-19 (Session 5 — **Phase 1 build**: analytical core + recipe safety scan + live end-to-end case; A-07 recorded) · prior same-day: Phase 0 CLOSED (Sessions 3–4, PRs #7–#9) · 2026-07-18 (Session 2 — Phase 0 build) · **Maintainer:** the loop; refreshed at every checkpoint.
...
**Phase 1 build (Session 5):** locks **1a/1b/1c/1d** green with executed evidence (see the Phase 1 inventory) — 1b run live end-to-end on the smoke model; A-07 schema amendment recorded (implemented; §6.2 canon edit pending upstream). Carried: CalibrationStatus gating + knowability-confidence blocking → Phase 2.5; 0e provenance/attestation → RC prep; human repo-settings item: required status checks on `main`. **Next after Phase 1 lands: Phase 2 — The Wall** (loop §6: Graphiti temporal graph, backend decision, AC-DL-1 §6 at lock 2b), fresh branch, explicit human go-ahead. Phase 0 record: CLOSED 2026-07-19 (PRs #7–#9). Every fresh session: `git fetch origin` first; goose is container-ephemeral (`scripts/build-goose.sh`, ~12 min).
